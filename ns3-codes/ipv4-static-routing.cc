  ...
  if (rtentry != 0)
    {
      ...
      /*added by gn 2018/6/19*/
      //get queue length
      Ptr<NetDevice> nd = rtentry->GetOutputDevice();
      Ptr<PointToPointNetDevice> p2pnd = StaticCast<PointToPointNetDevice> (nd);
      Ptr<Queue> queue = p2pnd->GetQueue ();
      uint16_t qSize = queue->GetNPackets ();
      //new a packet
      Ptr<Packet> packet = p->Copy();//Create<Packet>(1024);
      //add and set tag value
      myTag tag;
      p->PeekPacketTag(tag);

      uint16_t tmp1 = tag.GetValue3();
      if (qSize > tmp1) {
        tag.SetValue3(qSize);
      }
      NS_LOG_INFO (tmp1);
...
void myTag::Serialize(TagBuffer i) const 
{
  ...
  i.WriteU16(tagValue2);
  i.WriteU16(tagValue3);
}
...
void myTag::SetValue2(uint16_t value)
{
  tagValue2=value;
}

uint16_t myTag::GetValue2()
{
  return tagValue2;
}
void myTag::SetValue3(uint16_t value)
{
  tagValue3=value;
}

uint16_t myTag::GetValue3()
{
  return tagValue3;
}
